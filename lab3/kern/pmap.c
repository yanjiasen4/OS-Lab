...
#define psize_t sizeof(struct Page)
#define esize_t sizeof(struct Env)
// --------------------------------------------------------------
...
//	panic("mem_init: This function is not finished\n");
	...
	pages = boot_alloc(npages*psize_t);
	//////////////////////////////////////////////////////////////////////
	// Make 'envs' point to an array of size 'NENV' of 'struct Env'.
	// LAB 3: Your code here.

	envs = (struct Env *)boot_alloc(NENV*esize_t);
	//////////////////////////////////////////////////////////////////////
	...
	// Your code goes here:
	boot_map_region(kern_pgdir, UPAGES, ROUNDUP(npages*psize_t,PGSIZE),PADDR(pages), PTE_U | PTE_P);
	...
	// LAB 3: Your code here.
	boot_map_region(kern_pgdir, UENVS, ROUNDUP(NENV*esize_t,PGSIZE), PADDR(envs), PTE_U | PTE_P);
	//////////////////////////////////////////////////////////////////////
...
page_init(void)
{
	...
	size_t	pgstart = PGNUM(PADDR(boot_alloc(0)));
	for (i = pgstart; i < npages; i++) {
		pages[i].pp_ref = 0;
		pages[i].pp_link = page_free_list;
		page_free_list = &pages[i];
	}
...
{	
	...
	struct Page* ret_page = page_free_list;
...
// Return prev page
struct Page *
page_find_npages(int n)
{
	if (page_free_list == NULL) {
		return NULL;
	}
	struct Page *front_page = NULL;
	struct Page *head_page;
	struct Page *curr_page;
	struct Page *next_page;
	if (chunk_list != NULL) {
		head_page = chunk_list;
		curr_page = chunk_list;
		next_page = chunk_list->pp_link;
		int ncpage = 1;
		while(next_page != NULL) {
			if(ncpage == n) {
				goto FREE;
			}
			if(page2pa(curr_page) - page2pa(next_page) == PGSIZE) {
				ncpage++;
			} else {
				ncpage = 1;
				head_page = next_page;
				front_page = curr_page;
			}
			curr_page = next_page;
			next_page = next_page->pp_link;
		}
	}
	head_page = page_free_list;
	curr_page = page_free_list;
	next_page = page_free_list->pp_link;
	int ncpage = 1;
	while(next_page != NULL) {
		if(ncpage == n) {
			break;
...
FREE:
	{
		struct Page *tmp = head_page;
		int i;
		for(i = 1; i < n; i++) {
			tmp = tmp->pp_link;
		}
		if(front_page == NULL) {
			front_page = tmp->pp_link;
		} else {
			front_page->pp_link = tmp->pp_link;
		}
		tmp->pp_link = NULL;
	}
End:
...
struct Page *reverse_page(struct Page *p) {
	struct Page *re_tmp = NULL;
	struct Page *re_prev = NULL;
	while(p) {
		re_tmp = p->pp_link;
		p->pp_link = re_prev;
		re_prev = p;
...
{
	// Fill this function	
	struct Page *ret_page = page_find_npages(n);
	struct Page *tmp = ret_page;
	int i;
	if( (alloc_flags & ALLOC_ZERO) ) {
		for(i = 0; i < n-1; i++) {
	...
	// reserve return pages
	return reverse_page(ret_page);	
}
...
page_free(struct Page *pp)
{
	if (pp->pp_ref == 0) {
		...
		while(tmp) {
			...
			if(page2kva(tmp) > page2kva(pp)) {
				if(page2kva(tmp->pp_link) < page2kva(pp) ) {
					struct Page *tp = tmp->pp_link;
					tmp->pp_link = pp;
					pp->pp_link = tp;
					return;
				}
			} else if(page2kva(tmp) < page2kva(pp) ) {
...
page_realloc_npages(struct Page *pp, int old_n, int new_n)
{
	...
	int find = 1;
	if (old_n > new_n) {
		struct Page *new_head = pp+new_n-1;
		page_free_npages(new_head+1,old_n-new_n);
		new_head->pp_link = NULL;
	} else if(old_n < new_n) {
		int i;
		...
		// Allocate new page
		if(!find) {
			struct Page *new_p = page_alloc_npages(1,new_n);
			memmove(page2kva(new_p),page2kva(pp),PGSIZE*old_n);
			page_free_npages(pp,old_n);
			pp = new_p;
		}
		// Append page 
		if(find) {
			int i;
			struct Page *old_tail = pp+old_n-1;
			struct Page *tmp = old_tail;
			for(i = old_n; i < new_n; i++) {
				tmp->pp_link = pp+i;
				tmp = tmp->pp_link;
				memset(page2kva(tmp),0,PGSIZE);
			...
			// delete this pages from page_free_list
			struct Page *alloc_head = page_free_list;
		//	cprintf("a:%x\nb:%x\n",alloc_head,pp+old_n);
			while(alloc_head > pp+old_n) {
				if(alloc_head->pp_link == pp+old_n) {
					break;
				}
				alloc_head = alloc_head->pp_link;
			}
			tmp->pp_link = NULL;
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if(*pde & PTE_P) {
		ret_pte = (pte_t *)KADDR(PTE_ADDR(*pde)) + PTX(va);
	} else {
		if(create==0) {
			return NULL;
		}
...
{
	// Fill this function in
	struct Page *p = page_lookup(pgdir,va,NULL);

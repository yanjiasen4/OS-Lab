...
sys_sbrk(uint32_t inc)
{
	...
	//region_alloc(curenv, (void *)(curenv->env_sbrk - inc), inc);
	struct Page *p = NULL;
	char *va = ROUNDDOWN((char *) curenv->env_sbrk, PGSIZE);
	char *end = ROUNDDOWN((char *) (curenv->env_sbrk + inc - 1), PGSIZE);

	for(; va <= end; va += PGSIZE) {
		if (!(p = page_alloc(0)))
			panic("sys_sbrk failed!");
		page_insert(curenv->env_pgdir, p, va, PTE_P | PTE_U | PTE_W);
	}
	curenv->env_sbrk += inc;

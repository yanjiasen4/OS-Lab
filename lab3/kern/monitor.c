...
static struct Command commands[] = {
	...
	{ "c", "Continue, use in debug", mon_debug_c},
	{ "si", "Step by step, use in debug", mon_debug_si},
	{ "x", "Display memory, use in debug", mon_debug_x}
};
...
int
mon_debug_c(int argc, char **argv, struct Trapframe *tf)
{
	uint32_t eflags;
	if (tf == NULL) {
...
int
mon_debug_si(int argc, char **argv, struct Trapframe *tf)
{
	uint32_t eflags;
	if (tf == NULL) {
		cprintf("No trapped environment\n");
...
int
mon_debug_x(int argc, char **argv, struct Trapframe *tf)
{
	if (argc != 2) {
